...
function load_data() {
  ...
  var def = $.ajax({
    type: "POST",
    url: "../../mcpservices/SvOrder.asmx/load_WtOrder_Paging",
    contentType: "application/json; charset=utf-8",
    dataType: "json",
    ...
    success: function (response) {
      if (response.hasOwnProperty("d")) {
        ...
        if (objOd != null) {
          ...
          htm += "<th style='width:46px;'>付款</th>";
          htm += "<th>備註</th>";
          htm += "<th style='width:46px;'>重寄</th>";
          ...
          $.each(objOd, function (i, v) {
            ...
            if (v.IsPaid) {
              htm += "<td><span id='lbPaid" + id + "' style='color:#c0c0c0;'>已付</span></td>";
            } else {
              htm += "<td><span id='lbPaid" + id + "' style='color:red;'>未付</span></td>";
            }
            //htm += "<td style='width:120px;'><span id='lbDate" + id + "'>" + v.CreateDay.toString() + "</span></td>";
  ...
  def.done(function () {
    if (filterby == "Aid") {
      $.ajax({
        type: "POST",
        url: "../../mcpservices/SvOrder.asmx/Get_ActRegister_Amount",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
    ...
    success: function (data) {
      ...
      if (str.length > 0) {
        ...
        $.each(str, function (i, v) {
          ...
          htm += get_line(v.SeqNo.toString(), v.Name.toString(), v.SID.toString(), v.DOB.toString(), v.EmpID.toString(), v.Cell.toString(), v.Email.toString(), v.Price.toString(), v.Comment.toString());
        });
        htm += "<tr>";
        htm += "<td>";
...
function transfer_act(oid) {
  ...
  sb += "<select id='ddlTrans' class='form-control' style='width:320px!important;'>" + ddlActHtml + "</select>";
  sb += "</div>";
  sb += "<div class='input-zone'>";
  sb += "<button type='button' onclick='save_tran()' class='btn btn-danger'>轉換</button>";
  sb += "</div>";
  $(".details").html("");
  $(".details").html(sb);
  $("#imgLoad").hide();
...
function save_tran() {
  ...
  $.ajax({
    type: "POST",
    contentType: "application/json; charset=utf-8",
    url: "../../mcpservices/SvOrder.asmx/Transfer_wtOrder",
...
function save_main(id) {
  ...
  var isValid = check_input(tr.find("input[id^='txtCust']").val(), "09/09/1999", "A132587456", "4321", tr.find("input[id^='txtCel']").val(), tr.find("input[id^='txtTel']").val(), "3366", tr.find("input[id^='txtMail']").val(), tr.find("input[id^='txtPrice']").val());
  if (isValid != "OK") {
    alert(isValid);
    return false;
  }
  ...
  $.ajax({
    type: "POST",
    url: "../../mcpservices/SvOrder.asmx/Update_wt_Order",
    contentType: "application/json; charset=utf-8",
    dataType: "json",
...
function save_detail(seqno) {
  ...
  var isValid = check_input(tr.find("input[id^='txtEName']").val(), tr.find("input[id^='txtEDob']").val(), tr.find("input[id^='txtESid']").val(), tr.find("input[id^='txtEEmpID']").val(), tr.find("input[id^='txtECell']").val(), "02-2383-0294", "3366", tr.find("input[id^='txtEEmail']").val(), tr.find("input[id^='txtEPrice']").val());
  if (isValid != "OK") {
    msg.text(isValid);
    return false;
  }
  ...
  $.ajax({
    type: "POST",
    url: "../../mcpservices/SvOrder.asmx/Update_wt_OrderDetail",
    contentType: "application/json; charset=utf-8",
    dataType: "json",
...
function check_input(name, dob, sid, eid, cell, phone, ext, email, price) {
  ...
  if (!isTel(phone, "tel")) {
    result = "電話號碼格式錯誤！格式應為：02-8226-9088";
  }
  if (email.length <= 0) {
    result = "請輸入Email";
  }
...
function send_mail(id) {
  ...
  if (id.substring(0, 2) == "M1") url = "../../mcpservices/SvOrder.asmx/send_oneday_confirm_mail";

  if (id.substring(0, 2) == "M2") url = "../../mcpservices/SvOrder.asmx/send_twodays_confirm_mail";
...
function export_excel() {
  ...
  $.ajax({
    type: "POST",
    url: "../../mcpservices/SvOrder.asmx/Get_WtOrder_Report",
    contentType: "application/json; charset=utf-8",
    dataType: "json",
