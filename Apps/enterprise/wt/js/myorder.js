...
function load_my_order() {    
    var def = $.ajax({
        type: "POST",
        url: "../../mcpservices/SvOrder.asmx/load_MyOrder",
...
function generate_act_line() {
    ...
    //if (aryTwo[0].StartDay == "Y") {
    //           sb += '<input onclick="cancel(2)" type="button" class="btn btn-danger" value="取消" style="margin-right:6px;" />';
    //       } else {
    //           sb += '<span style="color:#c0c0c0; margin-right:6px;">[取消期限已過]</span>';
    //       }
    //       if (aryTwo[0].RegExpDay == "Y") {
    //           sb += '<input onclick="edit(2)" type="button" class="btn btn-success" value="修改" />';
    //       } else {
    //           sb += '<span style="color:#c0c0c0; margin-right:6px;">[修改期限已過]</span>';
    //       }

    //if (aryOne[0].StartDay == "Y") {
    //    sb += '<input onclick="cancel(1)" type="button" class="btn btn-danger" value="取消" style="margin-right:6px;" />';
    //} else {
    //    sb += '<span style="color:#c0c0c0; margin-right:6px;">[取消期限已過]</span>';
    //}
    //if (aryOne[0].RegExpDay == "Y") {
    //    sb += '<input onclick="edit(1)" type="button" class="btn btn-success" value="修改" />';
    //} else {
    //    sb += '<span style="color:#c0c0c0; margin-right:6px;">[修改期限已過]</span>';
    //}
...
function cancel(id) {
    var isok = confirm("確定要取消報名嗎？");
    if (isok == false) { return false; }
    var oid = "";
    var data = null;
    var capicaty = 0;
    if (id == 1) { target = "one"; oid = aryOne[0].OrderID; data = aryOne[0]; capicaty = aryOne.length; }
    if (id == 2) { target = "two"; oid = aryTwo[0].OrderID; data = aryTwo[0]; capicaty = aryTwo.length; }
    ...
    Order.Name = data.ActName;   
    Order.Email = data.Email;
    Order.Tel1 = data.Room;
    Order.Tel2 = data.ActID;
    Order.TotalPrice = capicaty;   
    //console.log(Order);
    //return false;
    ...
    $.ajax({
        type: "POST",
        url: "../../mcpservices/SvOrder.asmx/Cancel_My_Order",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
...
function submit() {
    ...
    var cdata = { "order": Order, "detail": arrayJoinner, "waiting": "A" };

    loading.show();
    
    if (target == "one") {
        if (oid.substring(0, 2) != "M1") {
