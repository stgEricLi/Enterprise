...
function confirmaion() {
    ...
    if (errorMsg.length > 0) { confirmaionError(errorMsg); return false; }
...
function create_order() {
    ...
    WT.makeOneDayWaitingList({ "order": Order, "detail": arrayJoinner, "waiting": "N" }).done(function (response) {
        ...
        if (response.hasOwnProperty('d')) {
            ...
            sb += "</p>";
            sb += "<p class='text-info'>";
            sb += "為了確保您的權益，煩請於三日內完成付款動作，您可使用 ATM 轉帳、匯款或其他現金存入的方式，將您的款項匯至：";
            sb += "</p>";
            sb += "<address>";
...
function put_waiting() {
    ...
    WT.makeOneDayWaitingList({ "order": Order, "detail": arrayJoinner, "waiting": "Y" }).done(function (response){
        //console.log(response);
        if (response.hasOwnProperty('d')) {
            var result = response.d;
            if (result.substring(0, 2) != "OK") {
            ...
            var sb = "";
            sb += "<p class='text-info'>";
            sb += "您已完成線上線上候補程序，您的候補編號為：<span class='emphsize'>" + oid + "</span>，此次候補應付總金額為：<span class='emphsize'>" + total + "</span>";
            //sb += "，報名細節請前往您的電子郵件信箱察看。系統已發送email到您所填寫的電子信箱中，若您沒收到此一信件請先檢查是否在您的垃圾郵件當中或直接與我們聯絡。";
            sb += "</p>";         
            $("#odcnt").html('');
            $("#odcnt").html(sb);
