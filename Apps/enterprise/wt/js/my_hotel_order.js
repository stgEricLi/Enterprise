  ...
  var def = $.ajax({
    ...
    success: function (response) {
      if (response.hasOwnProperty("d")) {
        ...
        if (totalrows <= 0) {
          ...
          if (objOd != null) {
            ...
            $.each(objOd, function (i, v) {
              ...
              htm += "<td>" + v.Comment.toString() + "</td>";
              htm += "<td style='text-align:center;'>";
              htm += "<button  id='del" + id + "' type='button' onclick='remove_waiting(this.id)' class='btn btn-danger btn-xs udpD'><span class='glyphicon glyphicon-remove'></span></button>";
              htm += "</td>";
              htm += "</tr>";
            });
            htm += "</table>";
        ...
        $("#waitingList").html(htm);
      } else {
        $("#waitingList").html("<p>您目前沒有任何訂房候補資訊！</p>");
      }
    },
...
function load_weekday() {
  console.log("load_weekday");
  var htm = "";
  $("#weekday").html("");
  ...
  $.ajax({
    ...
    contentType: "application/json; charset=utf-8",
    dataType: "json",
    data: JSON.stringify({ pageindex: 1, pagesize: 10, filterby: "EID", filtervalue: $("#txtEid").val(), extravalue: "" }),
    cache: false,
    success: function (response) {
      if (response.hasOwnProperty("d")) {
        var json = JSON.parse(response.d);
        ...
        if (totalrows <= 0) {
          ...
          if (objOd != null) {
            ...
            $.each(objOd, function (i, v) {
              ...
              htm += "<td>" + v.Email.toString() + "</td>";
              htm += "<td>" + v.RoomType.toString() + "人房</td>";
              htm += "<td>" + convertDate(v.AccessDate.toString()) + "</td>";
              htm += "<td>" + v.CheckIn.toString() + "</td>";
              htm += "<td>" + v.Price.toString() + "</td>";
              htm += "<td>" + v.Comment.toString() + "</td>";
              htm += "<td><button  id='del" + id + "' type='button' onclick='remove_weekday(this.id)' class='btn btn-danger btn-xs udpD'><span class='glyphicon glyphicon-remove'></span></button></td>";
              htm += "</tr>";
            });
            htm += "</table>";
            htm += "</div>";
        ...
        $("#weekday").html("<p>您目前沒有任何平日訂房申請資訊！</p>");
      }
    },
    error: function (xhr, textStatus, thrownError) {
      console.log(xhr.status);
...
function remove_waiting(id) {
  ...
  var tr = $("#" + id)
    ...
    .parent();
  id = id.replace("del", "");
  var img = $("#waiting_loading");

  if (id.length <= 0) {
    showMsg("無法取得候補編號，目前無法進行刪除！");
    return false;
  }
...
function remove_weekday(id) {
  ...
  var tr = $("#" + id)
    ...
    .parent();
  id = id.replace("del", "");
  var img = $("#weekday_loading");

  if (id.length <= 0) {
    showMsg("<p>無法取得申請編號，目前無法進行刪除！</p>");
    return false;
  }
